...
The current per-plan system has the right central idea: one persistent conductor binding must be selected jointly with every worker because its calls consume the same accounts, time, and quota. Each active project retains its own conductor session on that chosen binding. The current 112-hour, four-project case nevertheless should not be endorsed as operationally reliable. It is feasible only under modeled API-equivalent allowances, leaves little unallocated OpenAI or Anthropic headroom after stress reservation even though nominal expected usage is substantially lower, admits all forecast volume before requiring strong review evidence, and has an 11.04% objective-bound gap. The next design should preserve the fixed binding and shared accounting while replacing class-wide role assignments with integer task assignments, risk-qualified workflows, calibrated native meters, and receding-horizon dispatch.
...
The baseline forecasts `ceil(H × N / 4)` changes, where `H` is the user's wall-clock working window and `N` is the maximum concurrent project-conductor capacity. At `H=112` and `N=4`, it forecasts and admits 112 changes. This formula assumes linear full utilization; `N` does not prove that four projects remain busy for every hour and does not increase subscription capacity. A production demand model also needs active-project duty factors and arrival distributions learned from real work. The baseline distributes forecast demand into four file-oriented classes, reserves a nine-stage workflow, and selects one route for every worker-role/class pair. One fixed conductor spans the plan. Provider accounts share modeled allowances and serialize visits on provider calendars.

The baseline’s conductor is GPT-6 Astra low through a mapped Codex/OpenAI harness. It has cap one and Intelligence Index utility 0.459945. The plan contains 224 conductor workflow visits, but the resource model is 204 class-weighted Intelligence Index benchmark-task-equivalent units. Those are different quantities. The model charges 166.7728 expected and 208.4661 reserved API-equivalent dollars, with 5.4636 expected and 6.8295 reserved hours. It does not predict 224 literal native turns. A real conductor may make several model or tool calls in one workflow visit; the ledger must charge each actual call once.

| Account | Modeled weekly capacity | Expected usage | Reserved usage | Reserved headroom | Reserved hours |
|---|---:|---:|---:|---:|---:|
| OpenAI / Codex Pro 200 | 354.9231 | 227.1094 | 354.5181 | 0.4050 | 89.8151 |
| Anthropic / Claude Max 20× | 253.8462 | 134.8856 | 245.4098 | 8.4364 | 21.4160 |
| Muse / Power Usage 10× | 11.5385 | 0 | 0 | 11.5385 | 0 |
| Google / AI Ultra 20× | 92.2615 | 43.9929 | 80.8274 | 11.4342 | 6.6657 |
| xAI / SuperGrok Heavy | 69.2308 | 39.3326 | 49.1657 | 20.0650 | 5.2607 |

The expected and reserved columns serve different decisions. Expected usage estimates ordinary consumption. Reserved usage protects admission against the configured stress case. Actual usage is a ledger observation and may be expressed in native requests, tokens, premium requests, session limits, or another meter. The table’s capacity is a declared API-equivalent estimate, not a provider commitment. OpenAI is 99.886% reserved and Anthropic is 96.676% reserved in that artificial unit. Such narrow slack is unsuitable for automatic full-week admission until native telemetry establishes the mapping.

The worker routing also exposes objective mismatch. Standard and Complex implementation use GPT-5.6 Luna max with a maximum of three conditional attempts and cap-adjusted reference utility 0.8508; the underlying coding competence is about 0.686. Neither is a real-work success probability. Extensive implementation uses Gemini 3.7 Flash high with cap two and utility 0.7964. Focused implementation uses Grok 4.5 high with cap one and utility 0.7011. These may be defensible conditional choices, but the assurance roles are weak: every Debugger route uses Luna low, cap one, utility 0.3949; every Reviewer route uses Luna low, cap one, utility 0.3337. Focused Sanity uses Luna low at 0.1505. Complex Sanity raises Luna high to cap three and utility 0.4667. Standard Sanity selects Opus 5 medium, cap one, and reserves 204.8075 Anthropic units by itself. Extensive Sanity uses Luna low with cap three and utility 0.2636. Extensive Comprehension adds 40.6023 Anthropic units. A cap reserves the maximum conditional retry chain. It does not launch parallel replicas or require spending all attempts. Repeat only after a real failed acceptance, new repair evidence, or a distinct needed job.

Those utilities are weighted benchmark evidence. They should not be read as 15%, 33%, or 39% chances of catching an error. They still reveal that throughput can dominate assurance when no role qualification requirement is configured. The same Luna family also implements, debugs, reviews, checks, and comprehends much of the work. Correlated-error research warns that teams gain less from aggregation when members share failure modes; diversity helps only when it changes errors rather than labels [7](https://proceedings.mlr.press/v267/kim25e.html). The application’s retry correlation model does not establish independence between role passes or between similar model efforts.

Standard Sanity’s 204.8075-unit Anthropic reservation is not evidence that an alternative swap fits. Removing or replacing it changes the shared account, schedule, and quality objective; every proposal needs a complete feasible re-solve. Muse’s zero allocation is also not automatically waste. It can result from a small pool, missing role evidence, entitlement constraints, or correct opportunity-cost logic. Useful idle capacity is preferable to duplicate or low-value work.

The solver reports `feasible_search_limit`, quality 324.8814, upper bound 360.7344, and an 11.0357% relative gap after 256 nodes. All 112 changes are admitted, so the quantity objective is saturated. The route objective is not proven optimal. The bound is valid for the modeled objective and candidate set; it is not an 11% quality uncertainty interval. Coordinate improvement removes obvious single-route defects, but coupled substitutions may remain. The plan is therefore a feasible modeled baseline and a useful diagnostic, not a dependable production commitment. The low assurance scores indicate relative evidence gaps under the present scoring scheme; they do not prove the routes incapable.
...
## Decision on the 112-hour case

The current allocation should not be approved as a reliable operating plan. It is a useful modeled maximum-volume scenario showing that declared budgets, proxy times, and provider calendars can fit 112 forecast changes. It does not establish native quota sufficiency, whether 204 proxy units capture the native cost of 224 workflow visits, assurance adequacy, or optimal routing.

The immediate operating posture should be staged admission. Verify exact bindings, billing, native meters, reset windows, concurrency, and calendars. Admit a bounded set of ready tasks with risk-qualified workflows. Observe conductor and worker usage in native units. Replan after each meaningful settlement until resource distributions stabilize. Preserve slack on OpenAI and Anthropic rather than relying on 0.4050 and 8.4364 modeled units. Require evidence appropriate to debugging, review, and sanity before counting their capacity. Evaluate correlated checker choices where consequence warrants it. Treat the 11.04% solver gap as a reason to compare feasible coupled alternatives, not as a quality discount.

No model replacement can be recommended from the supplied totals alone. The Standard Sanity allocation cannot simply be swapped out, because its removal changes shared feasibility and assurance. Muse’s zero baseline may be correct. A stronger plan emerges from better demand, evidence, native-meter, and outcome data, followed by a joint solve. `H`, `N`, and subscription IDs underidentify the actual work: they say nothing about backlog readiness, consequence, tool needs, deadlines, account state, or error correlation.

The resulting system will still make uncertain decisions. Its advantage is that uncertainty is placed in explicit evidence, distributions, holds, bounds, and user choices. The conductor remains one accountable identity. The dispatcher owns arithmetic. Workers receive exact qualified bundles. Projects share scarce resources fairly. Research is invoked for decisions it can change. The objective is to realize the greatest useful value from the selected subscriptions during the working calendar, balancing depletion wherever qualified demand permits.

## Implementation handoff

Read this report, the repository `AGENTS.md`, and the current source before editing. Preserve the working tree and implement the joint, quality-qualified allocator with one fixed Orchestrator binding across `N` project sessions, shared native budgets, Net Research, budget pacing, and one versioned policy rendered consistently by the UI, Markdown, and runtime. Use GPT-5.6 Sol at low effort for implementation and mechanical work; use native Claude, Grok, Antigravity or Gemini, and Muse only for review or research when needed. Do not add or run tests. For implementation changes, run the applicable `cargo fmt`, offline clippy with warnings denied, and release-build gate. Defaults remain `H=40` and `N=1`; the current user case is `H=112`, `N=4`.

Do not commit, push, publish, or discard existing worktree changes. This report is the architectural specification. Empirical coefficients remain provisional until real-work telemetry supports them. Resolve documented routine implementation choices directly, while preserving explicit uncertainty and avoiding invented calibration, universal thresholds, or guarantees.
